...
class VisionZero extends React.Component {
  ...
  createTabHeaders(){
    return VISION_ZERO_CONTENT.map((content, index) => {
        ...
        return <Tab key={index} label={content.header} id={id} key={id} aria-controls={`vertical-tabpanel-${index}`}></Tab>
    })
  }
  ...
  createTabContent(){
    ...
    return VISION_ZERO_CONTENT.map((content, index) => {
      return <TabPanel key={index} value={currentTab} index={index} key={index}>
        <div>
